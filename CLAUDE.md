...
- **nydusify**: registry-side OCI→Nydus conversion (push flow). The Rust `nydusify/` crate is **in scope for v1.0 and is the converter tool going forward**, replacing the Go nydusify. All four subcommands are implemented and real (not stubs): `convert` (both `--oci-ref`/zran and standard modes, plus `--with-referrer` artifact push), `check`, `copy`, and `mount` — driven by the new `registry-client/` crate (OCI pull+push, bearer auth) plus `nydus-image`/`nydusd` subprocesses. Live end-to-end verification against a real registry is still pending. Separate from the node-local accel path, which uses `snapshotter/src/local_accel.rs` (no push) — nydusify is the registry-publish flow, not the transparent accel flow. See [docs/nydusify-rust.md](./docs/nydusify-rust.md) for usage.