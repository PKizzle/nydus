...
impl BlobRangeMap {
    ...
    pub fn new(blob_path: &str, count: u32, shift: u32) -> StorageResult<Self> {
    ...
    pub fn open(blob_id: &str, workdir: &str, count: u32, shift: u32) -> StorageResult<Self> {
    ...
    pub(crate) fn get_range(&self, start: u64, count: u64) -> StorageResult<(u32, u32)> {
...
impl RangeMap for BlobRangeMap {
    ...
    fn is_range_ready(&self, start: u64, count: u64) -> StorageResult<bool> {
    ...
    fn set_range_ready_and_clear_pending(&self, start: u64, count: u64) -> StorageResult<()> {
        if !self.is_range_all_ready() {
            let (start_index, end_index) = self.get_range(start, count)?;
