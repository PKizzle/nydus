...
impl Freelist {
  ...
  /// Reloads the freelist from a precomputed list of free ids (as produced by
  /// scanning the page tree in NoFreelistSync mode), preserving pending ids
  /// associated with currently-open transactions. Mirrors [`Self::reload`] but
  /// without a backing freelist page.
  pub(crate) fn reload_from_ids(&mut self, ids: &[PgId]) {
    self.read_ids(ids);
    let pcache: HashSet<PgId> = self
      .pending
      .iter()
