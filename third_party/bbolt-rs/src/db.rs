...
use crate::common::bucket::BUCKET_HEADER_SIZE;
...
/// Walks the page tree of `backend` (rooted at `meta.root().root()`) and
/// returns the sorted list of free pages in `[2, meta.pgid)` — every page id
/// in that range that is NOT reachable from the root bucket and is not one of
/// the two meta pages.
///
/// Mirrors Go bbolt's `db.freepages()` — the path used to reconstruct an
/// in-memory freelist for a database opened with `NoFreelistSync = true`,
/// where the on-disk meta records `PGID_NO_FREE_LIST` instead of a real
/// freelist page id.
///
/// The walk visits branch elements via `.pgid()` and leaf elements via the
/// `BucketHeader` stored in their value. Inline buckets (`BucketHeader::root
/// == 0`) live inside the parent leaf, so no extra pages are walked for them.
/// Overflow pages are marked reachable via `PageHeader::overflow`.
fn scan_free_pages(backend: &dyn DBBackend) -> Vec<PgId> {
  let meta = backend.meta();
  let high_water = meta.pgid().0;
  // 0 and 1 are the two meta pages.
  let mut reachable: HashSet<u64> = HashSet::with_capacity(high_water as usize);
  reachable.insert(0);
  reachable.insert(1);

  let root = meta.root().root();
  if root.0 != 0 && root.0 < high_water {
    let mut stack: Vec<PgId> = Vec::with_capacity(64);
    stack.push(root);
    while let Some(pgid) = stack.pop() {
      if pgid.0 >= high_water || reachable.contains(&pgid.0) {
        continue;
      }
      let page = backend.page(pgid);
      // Mark this page and any overflow pages as reachable.
      for i in 0..=page.overflow as u64 {
        reachable.insert(pgid.0 + i);
      }

      if page.is_branch() {
        if let Some(bp) = MappedBranchPage::coerce_ref(&page) {
          for elem in bp.elements() {
            stack.push(elem.pgid());
          }
        }
      } else if page.is_leaf() {
        if let Some(lp) = MappedLeafPage::coerce_ref(&page) {
          for i in 0..lp.count {
            if let Some(elem) = lp.get_elem(i) {
              if elem.is_bucket_entry() {
                let val = elem.value();
                if val.len() >= BUCKET_HEADER_SIZE {
                  // `pod_read_unaligned` so we don't trip the alignment check —
                  // a leaf entry's value lives at an arbitrary byte offset
                  // inside the page, which is not guaranteed to match
                  // `align_of::<BucketHeader>()`.
                  let header: BucketHeader =
                    bytemuck::pod_read_unaligned(&val[..BUCKET_HEADER_SIZE]);
                  // Non-inline sub-bucket — recurse into its root page.
                  if header.root().0 != 0 {
                    stack.push(header.root());
                  }
                  // Inline buckets store their tiny root page inline in `val`
                  // after the header — no separate pages to walk.
                }
              }
            }
          }
      ...
      // is_meta and is_free_list pages are never reachable from the bucket
      // tree, so we don't expect to land here.
    }
  }

  let mut free = Vec::with_capacity((high_water as usize).saturating_sub(reachable.len()));
  for i in 2..high_water {
  ...
  fn freelist(&self) -> MutexGuard<Freelist> {
    self
      ...
      .get_or_init(|| {
        ...
        if freelist_pgid == PGID_NO_FREE_LIST {
          // NoFreelistSync: reconstruct by scanning the page tree.
          let mut freelist = Freelist::new();
          let ids = scan_free_pages(self);
          freelist.read_ids(&ids);
          return Mutex::new(freelist);
        }
...
pub(crate) trait DbIApi<'tx>: 'tx {
  ...
  /// Whether the database was opened with `no_freelist_sync = true`.
  /// In NoFreelistSync mode the freelist is not persisted to disk: the meta
  /// page stores `PGID_NO_FREE_LIST` and the in-memory freelist is rebuilt by
  /// scanning the page tree on every open.
...
impl<'tx> DbIApi<'tx> for LockGuard<'tx, DbShared> {
  ...
  fn no_freelist_sync(&self) -> bool {
    match self {
      LockGuard::R(guard) => guard.no_freelist_sync(),
      LockGuard::U(guard) => guard.borrow().no_freelist_sync(),
    }
  }
}
  ...
  fn remove_rw_tx(&self, tx_closing_state: TxClosingState, rem_tx: TxId, tx_stats: Arc<TxStats>) {
    ...
    if tx_closing_state.is_rollback() {
      ...
      if tx_closing_state.is_physical_rollback() {
        ...
        if freelist_page_id != PGID_NO_FREE_LIST {
          ...
          // In NoFreelistSync mode the on-disk freelist is the sentinel — there
          // is no persisted page to reload. Reconstruct from the page tree
          // (same path as initial open).
          let scanned = scan_free_pages(self.backend.as_ref());
          freelist.reload_from_ids(&scanned);
        }
      }
    }
...
pub struct BoltOptions {
  ...
  #[builder(setter(
    ...
    When enabled, the on-disk meta page stores PGID_NO_FREE_LIST instead of a freelist page id, \
    and the in-memory freelist is reconstructed by scanning the page tree at open time \
    (matching Go bbolt's `NoFreelistSync` semantics)."
  ))]
...
impl Bolt {
  ...
  fn new_db(
    ...
    let meta = backend.meta();
    // PGID_NO_FREE_LIST is the on-disk sentinel for NoFreelistSync: the freelist
    // is reconstructed from a page-tree scan in the backend's `freelist()` init.
...
mod test {
  ...
  /// NoFreelistSync round-trip: open with `no_freelist_sync = true`, write
  /// data across two transactions (so the freelist must work in-memory across
  /// commits), close, reopen, and verify every value survives. Exercises both
  /// the commit-time PGID_NO_FREE_LIST write and the open-time page-tree
  /// reconstruction.
  #[test]
  #[cfg(not(any(miri, feature = "test-mem-backend")))]
  fn test_no_freelist_sync_round_trip() -> crate::Result<()> {
    use crate::common::defaults::PGID_NO_FREE_LIST;
    let opts = BoltOptions::builder().no_freelist_sync().build();
    let mut db = TestDb::with_options(opts)?;

    // First write tx — creates the bucket and inserts 1..=200.
    db.update(|mut tx| {
    ...
    // Second write tx — overwrites half and deletes the other half. This is
    // what forces the in-memory freelist to actually allocate from previously
    // freed pages on subsequent commits.
    db.update(|mut tx| {
      let mut b = tx.bucket_mut("data").unwrap();
      for i in 1u32..=200 {
        let k = format!("{:04}", i);
        if i % 2 == 0 {
          b.delete(&k)?;
    ...
    // Sanity check: after a commit in NoFreelistSync mode the meta page must
    // record PGID_NO_FREE_LIST as the freelist id, NOT a real page id. Read
    // the meta's `free_list` field straight off the file (offset matches the
    // `Meta` layout: u32 magic, u32 version, u32 page_size, u32 flags, u64
    // root_pgid, u64 sequence, then u64 free_list). The on-disk Meta begins
    // at PAGE_HEADER_SIZE (16) inside the meta page.
    {
      use crate::common::page::PAGE_HEADER_SIZE;
      use std::io::Read as _;
      let mut f = std::fs::File::open(db.tmp_file.as_ref().unwrap().path())?;
      let page_size = DEFAULT_PAGE_SIZE.bytes() as usize;
      let mut buf = vec![0u8; page_size * 2];
      f.read_exact(&mut buf)?;
      // Recover the active meta as the higher txid of {meta0, meta1}.
      let read_meta = |off: usize| -> (u64, u64) {
        let meta_start = off + PAGE_HEADER_SIZE;
        let txid_off = meta_start + 4 * 4 + 8 * 2 + 8 + 8;
        let freelist_off = meta_start + 4 * 4 + 8 * 2;
        let mut txid_bytes = [0u8; 8];
        let mut fl_bytes = [0u8; 8];
        txid_bytes.copy_from_slice(&buf[txid_off..txid_off + 8]);
        fl_bytes.copy_from_slice(&buf[freelist_off..freelist_off + 8]);
        (u64::from_le_bytes(txid_bytes), u64::from_le_bytes(fl_bytes))
      };
      let (t0, fl0) = read_meta(0);
      let (t1, fl1) = read_meta(page_size);
      let active_freelist = if t1 > t0 { fl1 } else { fl0 };
      assert_eq!(
        active_freelist,
        PGID_NO_FREE_LIST.0,
        "no_freelist_sync did not record the sentinel in meta"
      );
    }
    ...
    // After reopen, the in-memory freelist has been rebuilt by scanning the
    // page tree. Every odd key must survive with its v2 value; every even key
    // must be absent.
    db.view(|tx| {
      ...
      for i in 1u32..=200 {
        ...
        let got = b.get(&k);
        if i % 2 == 0 {
          assert!(got.is_none(), "even key {} should be deleted", k);
        } else {
          let want = format!("v2={}", i);
          assert_eq!(
            got.as_deref().map(String::from_utf8_lossy).as_deref(),
            Some(want.as_str()),
            "odd key {} value mismatch",
            k
          );
        }
      }
