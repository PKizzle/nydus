...
use crate::common::defaults::{IGNORE_NO_SYNC, PGID_NO_FREE_LIST};
...
impl<'tx> TxRwApi<'tx> for TxRwImpl<'tx> {
  ...
  fn commit(mut self) -> crate::Result<()> {
    ...
    let no_freelist_sync = Pin::as_ref(&self.db).guard().no_freelist_sync();
    {
      let new_bucket = self.tx.cell.bound().split_r().bucket_header;
      let mut tx = self.tx.cell.borrow_mut();
      tx.r.meta.set_root(new_bucket);

      // Free the prior on-disk freelist page so its slot can be reused. In
      // NoFreelistSync mode the freelist isn't persisted (meta records the
      // PGID_NO_FREE_LIST sentinel) and there is no real page to free.
      let old_freelist = tx.r.meta.free_list();
      if old_freelist != PGID_NO_FREE_LIST {
        let freelist_pg = tx.r.db.page(old_freelist);
    ...
    if no_freelist_sync {
      // NoFreelistSync: skip allocating + writing the freelist page; record the
      // sentinel in meta so the next open reconstructs the freelist by tree
      // scan. Matches Go bbolt's `Tx.Commit` NoFreelistSync branch.
      let mut tx = self.tx.cell.borrow_mut();
      tx.r.meta.set_free_list(PGID_NO_FREE_LIST);
    } else {
...
pub(crate) mod check {
  ...
  impl<'tx> TxICheck<'tx> for TxCell<'tx> {
    fn check(self) -> Vec<String> {
      ...
      // In NoFreelistSync mode the freelist isn't a real page — skip marking it.
